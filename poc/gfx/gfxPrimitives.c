...
to draw. \param g The green value of the arc to draw. \param b The blue value of
the arc to draw. \param a The alpha value of the arc to draw.
...
int arcRGBA(SDL_Renderer *renderer, Sint16 x, Sint16 y, Sint16 rad,
  ...
  /* 0 <= start & end < 360; note that sometimes start > end - if so, arc goes
   * back through 0. */
  while (start < 0)
    start += 360;
  while (end < 0)
  ...
  do {
    ...
    if (oct == startoct) {
      ...
      /* need to compute stopval_start for this octant.  Look at picture above
       * if this is unclear */
      dstart = (double)start;
      switch (oct) {
      case 0:
      ...
      /*
         ...
         The basic idea is that we're always changing drawoct after we draw, so
         we stop immediately after we render the last sensible pixel at x =
         ((int)temp). and whether to draw in this octant initially
       */
      if (oct % 2)
        drawoct |= (1 << oct); /* this is basically like saying drawoct[oct] =
    ...
    if (oct == endoct) {
      ...
      if (startoct == endoct) {
        ...
        /* otherwise: we only draw in this octant, so initialize it to false, it
         * will get set back to true */
        if (start > end) {

          /* unfortunately, if we're in the same octant and need to draw over
           * the whole circle, */
          /* we need to set the rest to true, because the while loop will end at
           * the bottom. */
          drawoct = 255;
        } else {
          drawoct &= 255 - (1 << oct);
  ...
  /* so now we have what octants to draw and when to draw them. all that's left
   * is the actual raster code. */
  ...
  do {
    ...
    if (cx > 0) {
      ...
      /* always check if we're drawing a certain octant before adding a pixel to
       * that octant. */
      if (drawoct & 4)
...
Note: Determines vertex array and uses polygon or filledPolygon drawing routines
to render.
...
value of the filled polygon to draw. \param b The blue value of the filled
...
int filledPolygonRGBAMT(SDL_Renderer *renderer, const Sint16 *vx,
                        ...
                        Uint8 a, int **polyInts, int *polyAllocated) {
  int result;
  int i;
  int y, xa, xb;
  int miny, maxy;
...
int filledPolygonColor(SDL_Renderer *renderer, const Sint16 *vx,
  ...
  Uint8 *c = (Uint8 *)&color;
  return filledPolygonRGBAMT(renderer, vx, vy, n, c[0], c[1], c[2], c[3], NULL,
                             NULL);
}
...
int _HLineTextured(SDL_Renderer *renderer, Sint16 x1, Sint16 x2, Sint16 y,
  ...
  if (w <= texture_w - texture_x_walker) {
    ...
    while (pixels_written < w) {
      ...
      dst_rect.w = source_rect.w;
      result |=
          (SDL_RenderCopy(renderer, texture, &source_rect, &dst_rect) == 0);
...
\param texture_dx the offset of the texture relative to the screeen. If you move
the polygon 10 pixels to the left and want the texture to apear the same you
need to increase the texture_dx value \param texture_dy see texture_dx \param
polyInts Preallocated temp array storage for vertex sorting (used for
...
int texturedPolygonMT(SDL_Renderer *renderer, const Sint16 *vx,
  ...
  if (!gfxPrimitivesPolyAllocated) {
    ...
    if (gfxPrimitivesPolyAllocated < n) {
      gfxPrimitivesPolyIntsTemp =
          (int *)realloc(gfxPrimitivesPolyInts, sizeof(int) * n);
      if (gfxPrimitivesPolyIntsTemp == NULL) {

        /* Realloc failed - keeps original memory block, but fails this
         * operation */
        return (-1);
      }
...
\param texture_dx the offset of the texture relative to the screeen. if you move
the polygon 10 pixels to the left and want the texture to apear the same you
need to increase the texture_dx value \param texture_dy see texture_dx
...
[character n] = [byte 1 row 1][byte 2 row 1]...[byte {pitch} row 1][byte 1 row
2] ...[byte {pitch} row height] where [byte n] = [bit 0]...[bit 7] where [bit n]
= [0 for transparent pixel|1 for colored pixel]

\param fontdata Pointer to array of font data. Set to NULL, to reset global font
to the default 8x8 font. \param cw Width of character in bytes. Ignored if
fontdata==NULL. \param ch Height of character in bytes. Ignored if
fontdata==NULL.
*/
...
Default is 0 (no rotation). 1 = 90deg clockwise. 2 = 180deg clockwise. 3 =
270deg clockwise. Changing the rotation, will reset the character cache.
...
int characterRGBA(SDL_Renderer *renderer, Sint16 x, Sint16 y, char c, Uint8 r,
  ...
  if (gfxPrimitivesFont[ci] == NULL) {
     ...
     */
    character =
        SDL_CreateRGBSurface(SDL_SWSURFACE, charWidth, charHeight, 32,
...
The spacing between consequtive characters in the string is the fixed number of
pixels of the character width of the current global font.
...
\param t Position for which to calculate interpolated value. t should be between
[0, ndata].

\returns Interpolated value at position t, value[0] when t<0, value[n-1] when
t>n.
*/
...
int thickLineRGBA(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2,
  ...
  if ((x1 == x2) && (y1 == y2)) {
    wh = width / 2;
    return boxRGBA(renderer, x1 - wh, y1 - wh, x2 + width, y2 + width, r, g, b,
                   a);
  }
